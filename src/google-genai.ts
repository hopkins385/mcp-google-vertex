...
export class GoogleGenAIService {
    ...
    try {
      ...
      for (const generatedImage of generatedImages) {
        ...
        if (imageData.gcsUri) {
          ...
          throw new Error('GCS URI handling not implemented. Please avoid using outputGcsUri.');
        }
   ...
   */
  async generateVideo(prompt: string, options: VideoGenerationOptions = {}): Promise<Buffer[]> {
    // Validate input
    if (!prompt || typeof prompt !== 'string' || prompt.trim().length === 0) {
      throw new Error('Prompt is required and must be a non-empty string');
    }
    ...
    if (prompt.length > 1000) {
      throw new Error('Prompt must be less than 1000 characters for video generation');
    }
    ...
    try {
      ...
      console.log(`Video generation started (${numberOfVideos} videos), waiting for completion...`);
  ...
  estimateImageCost(options: ImageGenerationOptions = {}): string {
    ...
    return `Estimated cost: $${totalCost.toFixed(4)} for ${numberOfImages} ${imageSize} image(s)`;
  }
  ...
  estimateVideoCost(options: VideoGenerationOptions = {}): string {
    ...
    const costPerSecond = resolution === '1080p' ? baseCostPer1080pSecond : baseCostPer720pSecond;
  ...
  validateImageOptions(options: ImageGenerationOptions): void {
    ...
    if (!validMimeTypes.includes(outputMimeType)) {
      throw new Error(`outputMimeType must be one of: ${validMimeTypes.join(', ')}`);
    }
  }
  ...
  validateVideoOptions(options: VideoGenerationOptions): void {
    ...
    if (options.referenceImages) {
      for (let i = 0; i < options.referenceImages.length; i++) {
        if (!options.referenceImages[i].referenceType) {
          throw new Error(`referenceImages[${i}]: referenceType is required`);
        }
      }
    }
  }
