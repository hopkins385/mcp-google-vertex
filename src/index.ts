...
import { resolveReferenceImage } from './tools/image-utils.js';
...
// A reference image is a plain string: local file path, gs:// URI, or http(s) URL.
const videoReferenceImageSchema = z
  .string()
  .describe(
    'Image source: local file path (/path/to/image.png), Cloud Storage URI (gs://bucket/path), or a URL (https://cdn.example.com/image.jpg)',
  );
...
async function main() {
      ...
      try {
        ...
        if (saveLocally) {
          ...
          for (let i = 0; i < imageBuffers.length; i++) {
            const fileExtension = options.outputMimeType === 'image/png' ? 'png' : 'jpg';
            const filename = storage.generateFilename(`image_${i + 1}`, fileExtension);
  ...
  mcpServer.registerTool(
    'generate-video',
    {
      ...
      inputSchema: {
        ...
        lastFrame: videoReferenceImageSchema
          .optional()
          .describe('Image to use as the last frame of generated videos (only for image-to-video)'),
        referenceImages: z
          .array(
            z.object({
          ...
          .optional()
          .describe('Images to use as references (up to 3 ASSET images or 1 STYLE image)'),
      },
      outputSchema: {
        success: z.boolean(),
        filePaths: z.array(z.string()).optional(),
    ...
    async ({
      ...
      const saveLocally = true;
      try {
        const resolvedLastFrame = lastFrame ? await resolveReferenceImage(lastFrame) : undefined;
        const resolvedReferenceImages = referenceImages
          ? await Promise.all(
              referenceImages.map(async (ref) => ({
                referenceType: ref.referenceType,
                image: await resolveReferenceImage(ref.image),
              })),
            )
          : undefined;
